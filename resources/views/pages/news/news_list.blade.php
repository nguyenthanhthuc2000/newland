...
@section('main')
    <div class="row">
        <div class="post-form-action col-12 pt-3">
            <h3>{{ $title }}</h3>
            <p>Hiện có {{ $lstNews->count() }} tin tức.</p>
            <div class="l__a">
                @if ($lstNews->count() == 0)
                    <strong>Không có bài viết.</strong>
                @else
                    @foreach ($lstNews as $l)
            ...
            <div class="paginate-styling">
                {{ $lstNews->appends(request()->only(['hinh-thuc', 'danh-muc', 'khu-vuc', 'muc-gia', 'dien-tich', 'tu', 'den']))->links() }}
            </div>
        </div>
    </div>
@endsection
